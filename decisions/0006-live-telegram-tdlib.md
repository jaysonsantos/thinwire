...
- Persistent vault keys (OS keychain when attached): optional `api_id` / `api_hash` override, session marker, TDLib database encryption key. Ephemeral (memory only): phone, code, 2FA password.
- How official builds obtain `api_id` / `api_hash` is [0007](0007-telegram-api-inject.md). This ADR does not embed a publisher pair in source.
...
- The unauthorized / stub banner drops only on `TelegramAuthPhase::Ready` (`authorizationStateReady`).
