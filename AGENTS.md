...
- Signal: out of v1 release builds (S2, amended by `0011`). Feature `signal-local` (#39) is available and local-only. It links Presage and libsignal. Release builds and OS zips never enable it. Default CI stays feature-off.
...
| `crates/thinwire-protocol/` | `ProtocolAdapter` trait, host channel, capability metadata, Critic risk strings, and the Telegram / Slack / WhatsApp / Discord adapters plus the local-only Signal adapter (`telegram/`, `slack/`, `whatsapp/`, `discord/`, `signal/`) |
