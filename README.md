...
v1 protocols are Telegram, WhatsApp (experimental), Discord (bot/OAuth inbox only), and Slack OAuth. Signal is out of v1 release builds. Local feature `signal-local` can link Presage and libsignal.
...
WhatsApp (`whatsapp-web`) and Signal (`signal-local`) are local-only AGPL features. Release builds and OS zips never enable them.
...
- Do not link AGPL code into release builds or OS zips. Local features `whatsapp-web` and `signal-local` are the exception (`0011-agpl-protocols-local-only`)
