...
v1 protocols are Telegram, WhatsApp (experimental), Discord (bot/OAuth inbox only), and Slack OAuth. Signal is out of v1 release builds. Local feature `signal-local` (#39) is available. The client is `crates/thinwire-signal` (AGPL-3.0-only). It links Presage and libsignal. It stays off by default.
...
WhatsApp (`whatsapp-web`) is a local-only AGPL feature. Signal (`signal-local`) is an available local-only feature (#39). Release builds and OS zips never enable them.

## License

MIT. Keep third-party notices (including Boost for TDLib if bundled). The binary embeds the Inter font (SIL OFL 1.1). See `crates/thinwire/assets/fonts/`. Release archives ship its license as `THIRD_PARTY_NOTICES/Inter-OFL.txt`. `crates/thinwire-signal` is AGPL-3.0-only. The app depends on that folder only with feature `signal-local`. Release builds do not enable it.
...
- Do not link AGPL code into release builds or OS zips. `whatsapp-web` is a local exception. Feature `signal-local` (#39) is the other exception (`0011-agpl-protocols-local-only`)
