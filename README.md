...
WhatsApp (`whatsapp-web`) is a local-only AGPL feature. Signal (`signal-local`) is a planned local-only feature (#39). Release builds and OS zips never enable them.
...
- Do not link AGPL code into release builds or OS zips. `whatsapp-web` is the local exception today. Planned feature `signal-local` (#39) is the other exception (`0011-agpl-protocols-local-only`)
...
- WhatsApp linked-device spike is feature `whatsapp-web` (pinned `whatsapp-rust`). It is local-only AGPL (`wacore-libsignal`, `0011`). Experimental and not the default UI. Default CI stays feature-off. Release builds and OS zips never enable it. Full-screen ToS/ban gate before QR or pair. Device store stays in app-data.
